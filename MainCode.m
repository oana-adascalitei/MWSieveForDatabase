...
ExtraF1 := [ 40, 164, 182, 205, 215, 300, 336 ]; //trivial torsion where we use aux_int = 1,2,4 
Failed := [];
...
//for index in [i : i in [47] | i in [295,333] eq false] do
  ...
  if (index in T3 and (3 in qc_pr[index] eq false)) or (index in T6 and (3 in qc_pr[index] eq false)) then
    AuxSet := [3]; 
  end if;
  ...
  for om in [1..#all_fake_coeffs[index][1]] do
    ...
    for ind in [[1,2],[1,3],[2,3],[1,2,3]] do 
      ...
      qc_M := &*[qc_primes[i]^exponents[i] : i in [1..#qc_primes]]; //modulus prior to adding aux_int 
      aux_int := 1; //the default aux_int
      print "aux_int =", aux_int;
      M := qc_M*aux_int; // modulus after aux_int
      ...
      qc_fake_coeffs_mod_M := coeffs_CRT(fake_coeffs, qc_primes, exponents);
      ...
      if d_f then
        done_pr[index][om] := qc_primes cat [aux_int];
        done_fake[index][om] := true;
        print d_f, qc_primes cat [aux_int];
        break ind;
      end if;
      ...
      if d_f eq false then //if aux_int = 1 does not succeed, try aux_int > 1
        for aux_int in AuxSet do 
          ...
          printf "number of cosets: %o\n", #Left; //new
          M := qc_M*aux_int;
          fake_coeffs_mod_M := combine_fake_good(Left, qc_M, aux_int); //we factor in aux_int > 1
          printf "after aux_int: %o\n", #fake_coeffs_mod_M;
          fake_coeffs_new  := fake_coeffs_mod_M;
          ...
          if d_f then //if the sieve is successful, we store the qc_primes and aux_int we used for each element of Omega
